  ...
  */
  ...
  parse = function(text) {
    ...
    if ((count_char(text, "(") !== count_char(text, ")")) || (count_char(text, "<") !== count_char(text, ">"))) {
      throw "ERREUR DE SYNTAXE: mal parenthésé";
    }
    words = [];
    word = null;
  ...
  draw_box = function(g, text, x, y, monospaced) {
    ...
    r.node.setAttribute('id',"rect_" +text)
    r.insertBefore(t);
    st = g.set(r, t);
  ...
  traverse_draw = function(g, obj, x, y) {
    ...
    traverse = function(obj, parallel) {
      ...
      if (is_arr(obj)) {
        ...
        if (obj != null) {
          mono = false; // C'est L'Axiome de depart
          if (obj !== Axiome) { // c'est un element terminal
            mono = true;
            if (obj.match(/^((\".*\")|(\'.*\'))$/) != null) {
  ...
  window.SyntaxDiagram = function(elem, code) {
    ...
    draw(elem, parsed);
    global_parser = parsed;
    return parsed;
  };

  window.generateSentences = function (graph) {
    time_parcous = 1; // le temps que met le cursor sur les graphes
    ...
    return time_parcous * 1.5;
  }

  function writeValue(json) {
    if (json !== null) {
      switch (typeof json){
        case "object":
          switch (typeof json.data) {
            case "string": // Cést une boucle
              if (json.type === "repeat") {
                writeValue(json.data); // Lors d'une boucle, on affiche toujours
                // la premiere branche qu'elle soit null ou pas
                if (json.args){
              ...
              break;
            case "object":
              if (json.type === "or") { // c'est un alternatif
                let branchesAlternatif = [];
              ...
              break;
            case "undefined": // un element en serie
              json.forEach(elt => writeValue(elt));
              break;
          }
          break;
